...
export class RevisarDocReemplazoComponent
  ...
  implements OnInit
{
  ...
  toGoRevisarReemplazoPersonalForm(row: PersonalReemplazo) {
    ...
    this.router.navigate([
      "/intranet/contratos/" +
        Link.REEMPLAZO_PERSONAL_REVIEW_FORM +
        "/" +
        encryptedId +
        "/" +
        idReemplazoPersonal,
  ...
  toGoBandejaContratos() {
    functionsAlert
      .questionSiNo("¿Desea ir a la bandeja de contratos?")
      .then((result) => {
    ...
    this.personalReemplazoService
      ...
      .subscribe((response) => {
        this.listPersonalReemplazo = response.content.filter(
          (item) =>
            item.estadoRevisarEval?.codigo === "BORRADOR" ||
            item.estadoRevisarEval?.codigo === "EN_PROCESO"
        );
      });
  }
