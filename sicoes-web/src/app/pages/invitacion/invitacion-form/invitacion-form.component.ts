...
export class InvitacionFormComponent extends BaseComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.invitacionService.obtenerPropuestaProfesional(idPropuestaProfesional, propuestaUuid).subscribe(inv => {
      const { nombre: estadoItem } = inv?.propuesta?.procesoItem?.estado;
      this.ESTADO_PROCESO_ITEM = estadoItem.toUpperCase();
  ...
  mostrarAlertaCaducado(){
    functionsAlertMod2.alertaConfigurable(`El item se encuentra en estado ${this.ESTADO_PROCESO_ITEM}, por lo que ya no es posible aceptar o rechazar la invitación`, 'Continuar', 'warning').then((result => {}))
  }
}
