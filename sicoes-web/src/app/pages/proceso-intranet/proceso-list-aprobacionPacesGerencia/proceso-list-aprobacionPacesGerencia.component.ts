...
import { PacesAprobarDivisionDTO, PacesObservarDivisionDTO } from 'src/app/interface/pace';

@Component({
  selector: 'vex-proceso-aprobacion-Paces-Gerencia',
  templateUrl: './proceso-list-aprobacionPacesGerencia.component.html',
  styleUrls: ['./proceso-list-aprobacionPacesGerencia.component.scss'],
  animations: [
    fadeInUp400ms,
    stagger80ms
  ]
})
export class ProcesoIntranetAprobacionPacesGerenciaComponent extends BasePageComponent<Solicitud> implements OnInit {
  ...
  cargarCombo() {
    this.parametriaService.obtenerMultipleListadoDetalle([
      ...
      this.listEstado = this.listEstado.filter(x => x.codigo == ListadoEnum.CONST_ESTADO_PACE_APROBADO_DIVISION ||
        x.codigo == ListadoEnum.CONST_ESTADO_PACE_APROBADO_ENVIADO ||
        x.codigo ==ListadoEnum.CONST_ESTADO_PACE_CANCELADO)
    })
  ...
  serviceTable(filtro) {
    return this.pacesService.buscarAsignacionG3Por(filtro)
  }
  ...
  CheckBoxVisible(row: any) {
    if (this.listEstado.find(x => x.idListadoDetalle == row.idTipoEstado).codigo == ListadoEnum.CONST_ESTADO_PACE_APROBADO_ENVIADO
    || this.listEstado.find(x => x.idListadoDetalle == row.idTipoEstado).codigo == ListadoEnum.CONST_ESTADO_PACE_CANCELADO) {
      return false;
    }
    return true;
  }
    ...
    try {
      ...
      functionsAlert.questionSiNo(msj).then((result) => {
        ...
        if (result.isConfirmed) {
          ...
          this.pacesService.aprobarEnviarMasivaPaceGerencia(listaPaces).subscribe(res2 => {
  ...
  cancelarPACE(row: any) {
    ...
    let msj = '¿Está seguro de que desea cancelar el PACE?'
    functionsAlert.questionSiNo(msj).then((result) => {

      if (result.isConfirmed) {
        this.pacesService.cancelarPACE(filtro).subscribe(res2 => {
          this.sleep(1000).then(any => {
            functionsAlert.success('cancelado correctamente').then((result) => {
              this.ngOnInit();
              // if (tipo == 'APROBADO') {
  ...
  validarAbilitarBotonCancelar(row: any) {
    if (this.listEstado.find(x => x.idListadoDetalle == row.idTipoEstado).codigo == ListadoEnum.CONST_ESTADO_PACE_APROBADO_DIVISION    
    ) {
      return true;
    }
  ...
  aprobarEnviar(row: any) {
    ...
    functionsAlert.questionSiNo(msj).then((result) => {
      ...
      if (result.isConfirmed) {
        this.pacesService.aprobarEnviar(filtro).subscribe(res2 => {
          this.sleep(1000).then(any => {
            functionsAlert.success('aprobado correctamente').then((result) => {
              this.ngOnInit();
              // if (tipo == 'APROBADO') {
              //   this.activarFirmaDigital();
              // }
