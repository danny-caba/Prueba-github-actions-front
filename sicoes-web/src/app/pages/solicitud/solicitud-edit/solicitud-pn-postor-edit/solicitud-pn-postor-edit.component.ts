...
import { LayoutDatosPersNatPostorComponent } from 'src/app/shared/layout-datos/layout-datos-pers-nat-postor/layout-datos-pers-nat-postor.component';
import Swal from 'sweetalert2';

@Component({
  selector: 'vex-solicitud-pn-postor-edit',
  templateUrl: './solicitud-pn-postor-edit.component.html',
  styleUrls: ['./solicitud-pn-postor-edit.component.scss'],
  animations: [
    fadeInUp400ms,
    stagger80ms
  ]
})
export class SolicitudPnPostorEditComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  @ViewChild('layoutDatosPersJur', { static: true }) layoutDatosPersJur: LayoutDatosPersNatPostorComponent;
  ...
  enviarSubsanacion() {
    ...
    functionsAlert.questionSiNo('¿Seguro que desea enviar la subsanación?').then((result) => {
      if (result.isConfirmed) {
        let formValues = this.obtenerDatos();
